...
import fs, {readdirSync, statSync, unlinkSync, existsSync, mkdirSync, readFileSync, rmSync, watch, promises} from 'fs'
...
conn.ev.on("group-participants.update", async ({ id, participants, action }) => {
  console.log("SE EJECUTO CAMBIOS DE PARTICIPANTES CORRECTAMENTE")
  async function rejectBlackList(jid) {
    if (!existsSync("./databases/blackListUsers.jsonl")) return null;
    try {
      const fileData = await promises.readFile("./databases/blackListUsers.jsonl", "utf-8");
      const lines = fileData.split("\n").filter((line) => line.trim() !== "");
      for (const line of lines) {
        const entry = JSON.parse(line);
        if (entry.jid === jid && entry.blacklist === true) {
          return entry;
        }
      }
      return null;
    } catch (err) {
      console.error("Error al leer blackListUsers.jsonl:", err);
      return null;
    }
  }

  const blackListText = (jid, reason) => {
    const username = `@${jid.split("@")[0]}`;
    return `${username} ha sido removido del grupo por estar en la lista negra.\nRazón: ${reason}\n\n${username}`;
  };

  if (action === "add") {
    console.log("SE AÑADIO UN PARTICIPANTE NUEVO")
    const groupMetadatax = await conn.groupMetadata(id);
    const participantsx = groupMetadatax.participants;
    const botJid = conn.user.lid.split(':')[0] + '@lid';
    const bot = participantsx.find((u) => u.id === botJid);
      console.log(bot)

    console.log(botJid)

    console.log("lid: ", conn?.user?.lid)

    console.log("jid: ", conn?.user?.jid)
    const isBotAdmin = bot?.admin || false;
    for (let user of participants) {
      const blacklistEntry = await rejectBlackList(user);
      if (blacklistEntry) {
        console.log("SE ENCONTRO COINCIDENCIA DE PARTICIPANTE EN LISTA NEGRA")
        console.log(user)
        if (!isBotAdmin) return console.log("El bot no es admin, retornando...")
        await conn.groupParticipantsUpdate(id, [user], "remove");
        await conn.sendMessage(id, { 
          text: blackListText(user, blacklistEntry.reason),
          mentions: [user, conn.user.id]
        });
        return;
      }
    }
